...
import ir.atriatech.pizzawifi.databinding.FragmentLotteriesBinding
import ir.atriatech.pizzawifi.entities.profile.lottery.LotteryModel


class LotteryFragment : BaseFragment(), RecyclerViewTools, IsEndOfRecyclerView, OnRefreshListener {
    lateinit var binding: FragmentLotteriesBinding
    private lateinit var mViewModel: LotteryFragmentViewModel

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        mViewModel = ViewModelProvider(this).get(LotteryFragmentViewModel::class.java)
        mObserver()
    }
    ...
    override fun onCreateView(
        ...
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_lotteries, container, false)
...
//        binding.toolbarId.setNavigationOnClickListener { requireActivity().onBackPressed() }
    ...
    override fun <T> onItemClick(position: Int, view: View, item: T) {
        item as LotteryModel
        val arg = extraArguments(item, ARG_STRING_1)
        navTo(R.id.lotteryDetailFragment, arg = arg)
    }
    ...
    private fun setAdapter() {
        if (mViewModel.mAdapter == null) {
            mViewModel.mAdapter = LotteryAdapter(this)
    ...
    private fun mObserver() {
        baseListObserver(
            ...
            object : RequestConnectionResult<MutableList<LotteryModel>> {
                ...
                override fun onSuccess(data: MutableList<LotteryModel>) {
