...
import ir.atriatech.pizzawifi.databinding.FragmentCampaignListBinding
import ir.atriatech.pizzawifi.entities.profile.campaign.Campaign


class CampaignListFragment : BaseFragment(), RecyclerViewTools, IsEndOfRecyclerView, OnRefreshListener {
    lateinit var binding: FragmentCampaignListBinding
    private lateinit var mViewModel: CampaignListFragmentViewModel

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        mViewModel = ViewModelProvider(this).get(CampaignListFragmentViewModel::class.java)
        mObserver()
    }
    ...
    override fun onCreateView(
        ...
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_campaign_list, container, false)
...
//        binding.toolbarId.setNavigationOnClickListener { requireActivity().onBackPressed() }
    ...
    override fun <T> onItemClick(position: Int, view: View, item: T) {
        item as Campaign
        val arg = extraArguments(item, ARG_PARCELABLE_1)
       navTo(R.id.campaignDetailFragment,arg)
    }
    ...
    private fun setAdapter() {
        if (mViewModel.mAdapter == null) {
            mViewModel.mAdapter = CampaignAdapter(this)
    ...
    private fun mObserver() {
        baseListObserver(
            ...
            object : RequestConnectionResult<MutableList<Campaign>> {
                ...
                override fun onSuccess(data: MutableList<Campaign>) {
